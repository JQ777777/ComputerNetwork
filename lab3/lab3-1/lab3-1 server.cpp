...
int RecvMessage(SOCKET& sockServ, SOCKADDR_IN& ClientAddr, int& ClientAddrLen, char* message)
{
    ...
    while (1)
    {
        ...
        {
            //判断是否接受的是别的包
            if (seq != int(packet1.head.seq))
            {
                //cout << seq << endl;
                //cout << ack << endl;
                //cout << packet1.head.seq << endl;
                //说明出了问题，返回ACK
                Packet packet4;
                ...
                cout << "服务器接收的序列号不一致，ack申请重传" << endl;
                //system("pause");
                continue;//丢弃该数据包
            }
    ...
    Packet packet3;
    char* Buffer3 = new char[sizeof(packet3)];
    packet3.head.flags = OVER;
    packet3.head.Check = 0;
    u_short temp = packetcheck((u_short*)&packet3, sizeof(packet3));
    packet3.head.Check = temp;
    memcpy(Buffer3, &packet3, sizeof(packet3));
    if (sendto(sockServ, Buffer3, sizeof(packet3), 0, (sockaddr*)&ClientAddr, ClientAddrLen) == -1)
    {
        cout << "发送结束信息错误" << endl;
        return -1;
    }
